...
sub sendMultipartOnce {
	...
	foreach my $to (@recips) {
		...
		#info("EMAIL SENT: [to: $to] $subject$attachinfo") if ($sent);
		#info("EMAIL FAILED: [to: $email] $subject$attachinfo") if (!$sent);
	}
...
sub sendMail {
	...
	#info("EMAIL SENT: [to: $email] $subject") if ($sent);
	#info("EMAIL FAILED: [to: $email] $subject") if (!$sent);
...
sub sendSMTP() {
	...
	my $remote = Net::SMTP->new("127.0.0.1",
				Hello => "edastro.com",
				Port => 26,
				Timeout => 10,
				) or return 0;
	...
	if ($remote) {
		...
		#info("SMTP: Failed to send mail to '$to'");
		return 0;
	}
}
