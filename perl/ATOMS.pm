...
BEGIN { # Export functions first because of possible circular dependancies
   ...
   $VERSION = 2.01;
   @ISA = qw(Exporter);
   @EXPORT_OK = qw(timeout make_csv parse_csv trim_csv epoch2maildate epoch2date date2epoch sec2string isDST 
			getValue btrim ltrim rtrim gmt2local local2gmt epoch2mysql epoch2datenodash switchHash MD5Date
			to_ascii randomize formatteddate2epoch epoch2formatteddate);
