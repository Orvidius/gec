...
BEGIN { # Export functions first because of possible circular dependancies
	...
	open DBCONF, "</www/EDtravelhistory/DB.conf";
	my @lines = <DBCONF>;
	close DBCONF;
...
sub master_mysql {
	...
	if (!$AutoReconnect && $dbh{$db} && !$dbh{$db}->ping()) {
		...
		info("MSQL DATABASE CONNECTION LOST: ($db) \"".join('","',cleanse($pass,@connectparams))."\" [$sql_show]-- $@");
		delete($dbh{$db});
	}
	...
	if (!$dbh{$db}) {
		while (!$dbh{$db} && $count < 5) {
			...
			$count++;
			info("CONNECT FAILED: Retrying [$count] ($db) \"".join('","',cleanse($pass,@connectparams))."\" [$sql_show]-- $@") if (!$dbh{$db});
		}
	} else {
		$connected = 1;
	}

	info("CONNECT FAILED: Giving Up! [$count attempts] ($db) \"".join('","',cleanse($pass,@connectparams))."\" [$sql_show]-- $@") if (!$connected);
	die "CONNECT FAILED: Giving Up! [$count attempts] ($db) [$sql_show]-- $@\n" if (!$connected);
